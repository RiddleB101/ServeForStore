    ...
    if pay_list:
        ...
        if pay_order_items_map:
            ...
            for item in pay_order_items_map:
                tmp_product_ids = selectFilterObj(pay_order_items_map[item], "product_id")
                tmp_product_ids = {}.fromkeys(tmp_product_ids).keys()
                product_ids = product_ids + list(tmp_product_ids)

            # product_ids里面会有重复的，要去重
            product_mapping = getDictFilterField(Product, Product.id, "id", product_ids)
        ...
        for item in pay_list:
            ...
            for tmp_order_item in tmp_order_items:
                tmp_product_info = product_mapping[tmp_order_item.product_id]
                tmp_products.append({
                    'name': tmp_product_info.name,
                    'quantity': tmp_order_item.quantity
                })

            tmp_data['products'] = tmp_products
...
def info():
    ...
    if order_item_list:
        product_map = getDictFilterField(Product, Product.id, "id", selectFilterObj(order_item_list, "product_id"))
